...
namespace itLab1
{
    public class dbManager
    {
        ...
        public Table SearchInTable(int tIndex, string text)
        {
            Table resTable = new Table("res");

            resTable.tColumnsList.AddRange(db.dbTablesList[tIndex].tColumnsList);
            for (int i = 0; i < db.dbTablesList[tIndex].tRowsList.Count; ++i)
            {
                bool contains = false;
                for (int j = 0; j < db.dbTablesList[tIndex].tRowsList[i].rValuesList.Count; ++j)
                {
                    if (db.dbTablesList[tIndex].tRowsList[i].rValuesList[j].Contains(text))
                    {
                        contains = true;
                        break;
                    }
                ...
                if (contains)
                {
                    resTable.tRowsList.Add(db.dbTablesList[tIndex].tRowsList[i]);
                }
            }
